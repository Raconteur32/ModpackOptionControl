...
@Composable
fun DraftPanel(state: AppState) {
    val draftEntries = state.draftEntries
    val listState    = rememberLazyListState()

    LaunchedEffect(state.draftIndex) {
        if (draftEntries.isNotEmpty())
            listState.scrollToItem(state.draftIndex.coerceAtMost(draftEntries.size - 1))
    }
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        ...
        Row(
            ...
            Text("DRAFT", fontWeight = FontWeight.Bold, fontSize = 13.sp)
            Spacer(Modifier.width(8.dp))
            Text(
                "${draftEntries.size} pending entr${if (draftEntries.size != 1) "ies" else "y"}",
                color = Color.Gray, fontSize = 11.sp
            )
            Spacer(Modifier.weight(1f))
            if (draftEntries.isNotEmpty()) {
                Button(
                    onClick = { state.finalizeDialogVisible = true },
        ...
        if (draftEntries.isEmpty()) {
            Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
                Text("No pending entries.", color = Color.Gray, fontSize = 12.sp)
            }
        } else {
            LazyColumn(state = listState, modifier = Modifier.fillMaxSize()) {
                itemsIndexed(draftEntries) { i, entry ->
                    val selected   = state.focusedPanel == FocusedPanel.Draft && i == state.draftIndex
